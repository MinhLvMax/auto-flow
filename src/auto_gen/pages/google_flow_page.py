...
class GoogleFlowPage(BasePage):
    ...
    def _is_exist_project(self, project_name):
        return self._page.get_by_text(f"{project_name}editEdit project").is_visible()

    def _get_project_page(self):
        project_page = GoogleFlowProjectPage(self._page)
    ...
    def _open_exist_project_page(self, project_name):
        span = self._page.get_by_text(f"{project_name}editEdit project")
